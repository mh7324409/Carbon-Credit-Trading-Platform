...
(define-private (batch-transfer-iter (item {credit-id: uint, amount: uint}) (result (response principal uint)))
    (let ((recipient (try! result)))
        (match (transfer-credits recipient (get amount item) (get credit-id item))
            success (ok recipient)
            error (err error)
        )
    )
)

(define-public (batch-transfer-credits (recipient principal) (transfers (list 10 {credit-id: uint, amount: uint})))
    (match (fold batch-transfer-iter transfers (ok recipient))
        success (ok true)
        error (err error)
    )
)
